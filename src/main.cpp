...
// 读取CSV并解析数据
bool readCSV(const string& filename, vector<vector<double>>& currentData, vector<double>& weights) {
    ...
    while (getline(file, line)) {
        ...
        while (getline(ss, cell, ',')) {
            try {
                ...
                cerr << "Error: Invalid number in CSV file: " << cell << endl;
                return false;  // 转换失败
            }
        }
    ...
    // 检查每行数据的列数是否一致，避免越界问题
    size_t numColumns = weights.size();
...
void processCSVData(const string& filename) {
    ...
    // 读取CSV数据
    if (!readCSV(filename, currentData, weights)) {
        return;  // 如果读取失败，返回
    }
